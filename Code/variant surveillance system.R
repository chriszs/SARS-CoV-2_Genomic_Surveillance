...
us.dat = merge(us.dat,
               pangolin[, c("nt_id", "lineage")],
               by.x = "primary_nt_id",
               by.y = "nt_id",
               all.x = TRUE)
if ("covv_lineage" %in% names(us.dat))
  us.dat$lineage = with(us.dat, ifelse(is.na(lineage), covv_lineage, lineage))
# NS3 identifier in baseline$source:
us.dat = merge(
  us.dat,
  baseline[, c("nt_id", "source", "primary_virus_name", "s_mut")],
...
tests$yr_wk = as.character(tests$collection_date - as.numeric(strftime(tests$collection_date, format =
                                                                         "%w")))
tests$TOTAL = rowSums(tests[, c("INDETERMINATE", "INVALID", "NEGATIVE", "POSITIVE")], na.rm =
                        TRUE)
tests$POSITIVE = ifelse(is.na(tests$POSITIVE), 0, tests$POSITIVE)
# Aggregate by week for weighting
tests_wk = expand.grid(
  STUSAB = unique(tests$STUSAB),
  yr_wk = unique(tests$yr_wk),
  stringsAsFactors = FALSE
)
for (cc in c("POSITIVE", "TOTAL")) {
  tests_wk = merge(tests_wk, data.frame(xtabs(tests[, cc] ~ STUSAB + yr_wk, tests)), all.x =
                     TRUE)
...
test_tallies_wk = merge(
  merge(tests_wk, hhs, all.x = TRUE),
  data.frame(
...
test_tallies_wk = within(test_tallies_wk,
                         INFECTIONS <-
                           ifelse(TOTAL > 0, POSITIVE * sqrt(state_population / TOTAL), 0))
test_tallies_wk$week = as.numeric(as.Date(test_tallies_wk$yr_wk) - week0day1) %/%
  7
...
sgtf.sub = unique(subset(
  us.dat,
  contractor_targeted_sequencing %in% "Screened for S dropout"
)[, c("primary_state_abv", "yr_wk")])
sgtf.glm = glm(
...
sgtf.sub$sgtf_weights = predict(sgtf.glm, cbind(contractor_vendor_name =
                                                  c("Helix/Illumina"), sgtf.sub), type = "response") /
  predict(sgtf.glm, cbind(contractor_vendor_name = c("Other"), sgtf.sub), type =
            "response")
test_tallies_wk = merge(
  test_tallies_wk,
  sgtf.sub,
...
test_tallies_wk$sgtf_weights = ifelse(is.na(test_tallies_wk$sgtf_weights),
                                      1,
                                      test_tallies_wk$sgtf_weights) # Inconsequential, but avoids disruptions due to NA
...
svy.dat = data.frame(
  ...
  SGTF_UPSAMPLING = (
    us.dat$contractor_targeted_sequencing %in% "Screened for S dropout"
  ),
...
svy.dat = merge(svy.dat,
                cbind(
                  test_tallies_wk[, c("STUSAB", "yr_wk")],
                  I_over_POSITIVE_unadj = sqrt(test_tallies_wk$state_population /
                                                 test_tallies_wk$TOTAL)
                ),
                all.x = TRUE)
...
svy.dat = merge(svy.dat, test_tallies_wk[, c("STUSAB",
                                             "yr_wk",
                                             "POSITIVE",
                                             "TOTAL",
                                             "INFECTIONS",
                                             "sgtf_weights")], all.x = TRUE)
svy.dat$sgtf_weights[is.na(svy.dat$sgtf_weights) |
                       !svy.dat$SGTF_UPSAMPLING] = 1
svy.dat = merge(svy.dat, incidence_by_region[, c("HHS", "yr_wk", "HHS_INCIDENCE")], all.x =
                  TRUE)
...
for (vv in fac2str)
  svy.dat[, vv] = as.character(svy.dat[, vv])
...
src.dat = subset(svy.dat, SOURCE != "OTHER" &
                   !is.na(VARIANT) &
                   VARIANT != "None") # VARIANT exclusions added 2021-04-21
src.dat$sgtf_weights[is.na(src.dat$sgtf_weights) |
                       !src.dat$SGTF_UPSAMPLING] = 1 # svy.dat variable redefinition resolves this
...
# Just to be sure:
src.dat = subset(src.dat,!is.na(SIMPLE_ADJ_WT) &
                   SIMPLE_ADJ_WT < Inf)
src.dat$NORM_WTS = with(src.dat, SIMPLE_ADJ_WT / sum(SIMPLE_ADJ_WT, na.rm =
                                                       TRUE))
...
myciprop = function(voc,
                    ...
                    ...) {
  if (mut) {
    VOC = (unlist(gregexpr(voc, svy$variables$S_MUT)) != -1)
  } else {
    VOC = (svy$variables$VARIANT %in% voc)
  }
  res = svyciprop( ~ VOC, design = subset(
    update(svy, VOC = VOC),
    (STUSAB %in% geoid) | (HHS %in% geoid) | (geoid == "USA")
  ), ...)
...
src.dat$K_US = sapply(1:nrow(src.dat), function(nn)
  which(c(names(model_vars), src.dat$VARIANT[nn]) == src.dat$VARIANT[nn])[1])

# National, unadjusted
get_cis = TRUE # Hessian slow!
temp_us = multinom(
  K_US ~ I(week - current_week),
  data = subset(src.dat, week >= current_week - model_weeks),
  weights = NORM_WTS,
  Hess = get_cis,
  maxit = 1000,
  trace = FALSE
...
temp_hhs = multinom(
  K_US ~ I(week - current_week) + as.factor(HHS),
  data = subset(src.dat, week >= current_week - model_weeks),
  weights = NORM_WTS,
  Hess = get_cis,
  maxit = 1000,
...
se.multinom = function(mlm, week, geoid = "USA") {
  ...
  vc.y_i = outer(1:dim(sub.vc)[2], 1:dim(sub.vc)[4], Vectorize(function(i, j)
    c(coeffs %*% sub.vc[, i, , j] %*% coeffs)))
...
qmbinom = function(mu, sigma, size, p) {
  if (is.na(sigma) |
    ...
    findInterval(p, cumsum(dmbinom(mu, sigma, size, 0:size)))
  }
}
...
ests = data.frame(
  ...
  Proportion = ests$p_i,
  se.Proportion = ests$se.p_i
)
ci = apply(ests, 1, function(rr)
  qmbinom(as.numeric(rr["Proportion"]), as.numeric(rr["se.Proportion"]), df, c(0.025, 0.975)) /
    df)
